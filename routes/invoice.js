...
router.post("/multiple", function (req, res, next) {
  connection.query(
    "SELECT *, IFNULL((SELECT JSON_ARRAYAGG(json_object('itemId',itemId,'discount',discount,'discountTypeId',discountTypeId,'count',count,'price',price,'total',total,'discountName',(SELECT discountName FROM discount WHERE idDiscount = invoiceContent.discountTypeId LIMIT 1),'itemName',(SELECT  IFNULL(CONCAT(itemType , ' ' , itemName,' ' , itemWeight, ' ' ,itemWeightSuffix, ' ' , ' * ' , cartonQauntity , ' ' , brand.brandName), item.itemName)  FROM item LEFT JOIN itemGroup ON item.itemGroupId = itemGroup.idItemGroup LEFT JOIN brand ON item.brandId = brand.idBrand LEFT JOIN itemType ON itemType.idItemType = item.itemTypeId WHERE idItem = invoiceContent.itemId LIMIT 1))) FROM invoiceContent WHERE invoice.idInvoice = invoiceContent.invoiceId),'[]') As items , (SELECT customerName FROM customer WHERE idCustomer = invoice.customerId) As customerName, (SELECT storeName FROM customer WHERE idCustomer = invoice.customerId) As storeName,(SELECT phone FROM customer WHERE idCustomer = invoice.customerId) As customerPhone,(SELECT secondPhone FROM customer WHERE idCustomer = invoice.customerId) As secondCustomerPhone,(SELECT nearBy FROM customer WHERE idCustomer = invoice.customerId) As customerAddress, (SELECT username FROM user WHERE idUser = invoice.createdBy) As createdByName, (SELECT username FROM user WHERE idUser = invoice.deliveryId) As deliveryName, (SELECT COALESCE(SUM(total),0) FROM invoiceContent WHERE invoiceId = invoice.idInvoice) As totalPrice, (SELECT COUNT(*) FROM invoiceContent WHERE invoiceId = invoice.idInvoice) As totalItems, (SELECT COUNT(*) FROM damagedItemsInvoice WHERE damagedItemsInvoice.customerId = invoice.customerId AND DATE(damagedItemsInvoice.createdAt) = DATE(invoice.createdAt)) As totalDamaged, DATE_FORMAT(createdAt, '%Y-%m-%d') As creationFixedDate, DATE_FORMAT(createdAt, '%T') As creationFixedTime, DATE_FORMAT(createdAt, '%W') As creationDayName FROM invoice JOIN invoiceType ON invoice.invoiceTypeId = invoiceType.idInvoiceType WHERE idInvoice IN (?) ORDER BY invoice.createdBy, invoice.idInvoice ASC",
    [req.body.invoices],
    (err, result) => {
      console.log(err);
      if (result.length > 0) {
...
router.get("/filter", function (req, res, next) {
  ...
  connection.query(
    `SELECT *,(SELECT customerName FROM customer WHERE idCustomer = invoice.customerId) As customerName,(SELECT storeName FROM customer WHERE idCustomer = invoice.customerId) As storeName,(SELECT phone FROM customer WHERE idCustomer = invoice.customerId) As customerPhone,(SELECT secondPhone FROM customer WHERE idCustomer = invoice.customerId) As secondCustomerPhone,(SELECT nearBy FROM customer WHERE idCustomer = invoice.customerId) As customerAddress,(SELECT username FROM user WHERE idUser = invoice.createdBy) As createdByName, (SELECT location FROM customer WHERE idCustomer = invoice.customerId) As customerLocation, (SELECT visitDay FROM customer WHERE idCustomer = invoice.customerId) As visitDay, (SELECT username FROM user WHERE idUser = invoice.deliveryId) As deliveryName, (SELECT COALESCE(SUM(total),0) FROM invoiceContent WHERE invoiceId = invoice.idInvoice) As totalPrice, (SELECT COUNT(*) FROM invoiceContent WHERE invoiceId = invoice.idInvoice) As totalItems, DATE_FORMAT(createdAt, '%Y-%m-%d') As creationFixedDate, DATE_FORMAT(createdAt, '%r') As creationFixedTime, DATE_FORMAT(createdAt, '%W') As creationDayName FROM invoice JOIN invoiceType ON invoice.invoiceTypeId = invoiceType.idInvoiceType WHERE 1=1 ${query} ${order} ${limit}`,
    (err, result) => {
      res.send(result);
      console.log(err);
    },
