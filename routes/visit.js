...
router.get('/user/:id', function (req, res, next) {
    connection.query(`SELECT visit.*, customer.customerName, customer.storeName , customer.visitDay, customer.secondVisitDay , visitCause.visitCauseName ,DATE_FORMAT(visit.createdAt, '%Y-%m-%d %r') As creationFixedDate, DATE_FORMAT(visit.createdAt, '%W') As creationDayName FROM visit JOIN user ON visit.createdBy = user.idUser JOIN customer ON customer.idCustomer = visit.customerId JOIN visitCause ON visitCause.idVisitCause = visit.visitCauseId WHERE visit.createdBy = ? AND DATE(visit.createdAt) BETWEEN '${req.query.dateFrom}' AND '${req.query.dateTo}'`,[req.params.id], (err, result) => {
        res.send(result);
        if (err) {
            console.log(err);
        }
