...
router.get("/userid/:id", function (req, res, next) {
  ...
  connection.query(
    `SELECT *, (SELECT username FROM user WHERE idUser = supervisorDelegates.delegateId) As delegateName , (SELECT @totalInvoices := IFNULL(COUNT(idInvoice),0) FROM invoice WHERE invoice.invoiceTypeId = 1 AND createdBy = supervisorDelegates.delegateId AND DATE(createdAt) = '${date}') As totalInvoicesToday, (SELECT @totalInvoices := IFNULL(COUNT(idInvoice),0) FROM invoice JOIN customer ON customer.idCustomer = invoice.customerId WHERE invoice.invoiceTypeId = 1 AND invoice.createdBy = supervisorDelegates.delegateId AND DATE(invoice.createdAt) = '${date}' AND customer.visitDay != '${dayname}') As totalInvoicesTodayNotInRail, (SELECT @totalVisits := IFNULL(COUNT(idVisit),0) FROM visit WHERE createdBy = supervisorDelegates.delegateId AND DATE(createdAt) = '${date}') As totalVisitsToday, (SELECT @totalVisits := IFNULL(COUNT(idVisit),0) FROM visit JOIN customer ON visit.customerId = customer.idCustomer WHERE visit.createdBy = supervisorDelegates.delegateId AND DATE(visit.createdAt) = '${date}' AND customer.visitDay != '${dayname}') As totalVisitsTodayNotInRail, (SELECT @totalCustomerToCheck := IFNULL(COUNT(idCustomer),0) FROM customer WHERE createdBy = supervisorDelegates.delegateId AND (visitDay = LOWER(DAYNAME('${date}')) || secondVisitDay = LOWER(DAYNAME('${date}')))) As totalCustomersToCheck, (@totalCustomerToCheck - @totalInvoices - @totalVisits) As ramainingCustomers, (SELECT @totalBills := IFNULL(sum(total),0) FROM invoiceContent JOIN invoice ON invoiceContent.invoiceId = invoice.idInvoice WHERE invoice.createdBy = supervisorDelegates.delegateId AND invoice.invoiceTypeId = 1 AND DATE(invoice.createdAt) = '${date}') As totalBills,(SELECT @totalRestoreBills := IFNULL(sum(total),0) FROM invoiceContent JOIN invoice ON invoiceContent.invoiceId = invoice.idInvoice WHERE invoice.createdBy = supervisorDelegates.delegateId AND invoice.invoiceTypeId = 3 AND DATE(invoice.createdAt) = '${date}') As totalRestoreBills , (SELECT @totalDamaged := IFNULL(SUM(totalPrice),0) FROM damagedItemsInvoiceContents JOIN damagedItemsInvoice ON damagedItemsInvoiceContents.damagedItemsInvoiceId = damagedItemsInvoice.idDamagedItemsInvoice WHERE damagedItemsInvoice.createdBy = supervisorDelegates.delegateId AND DATE(damagedItemsInvoice.createdAt) = '${date}') As totalDamaged FROM supervisorDelegates WHERE supervisorId = ? ORDER BY @totalBills ASC`,
    [req.params.id],
    (err, result) => {
      res.send(result);
      if (err) {
