...
router.get('/contents', function (req, res, next) {
    ...
    connection.query(`SELECT *, (SELECT @customerId := customerId FROM damagedItemsInvoice WHERE idDamagedItemsInvoice = damagedItemsInvoiceContents.damagedItemsInvoiceId) As customerId,(SELECT storeName FROM customer WHERE idCustomer = @customerId) As customerName, (SELECT  IFNULL(CONCAT(itemType , ' ' , itemName,' ' , itemWeight, ' ' ,itemWeightSuffix, ' ' , ' * ' , cartonQauntity , ' ' , brand.brandName), item.itemName)  FROM item LEFT JOIN itemGroup ON item.itemGroupId = itemGroup.idItemGroup LEFT JOIN brand ON item.brandId = brand.idBrand LEFT JOIN itemType ON itemType.idItemType = item.itemTypeId WHERE idItem = damagedItemsInvoiceContents.itemId) As itemName ,(SELECT imagePath FROM item WHERE idItem = damagedItemsInvoiceContents.itemId) As imagePath , DATE_FORMAT(createdAt, '%Y-%m-%d') As creationFixedDate, DATE_FORMAT(createdAt, '%T') As creationFixedTime, DATE_FORMAT(createdAt, '%W') As creationDayName , (SELECT @createdBy := createdBy FROM damagedItemsInvoice WHERE idDamagedItemsInvoice = damagedItemsInvoiceContents.damagedItemsInvoiceId) As createdBy, (SELECT username FROM user WHERE idUser = @createdBy) As createdByName FROM damagedItemsInvoiceContents WHERE 1=1 ${query} ${order} ${limit}`, (err, result) => {
        res.send(result);
        if (err) {
            console.log(err);
        }
...
router.post('/new', function (req, res, next) {
    connection.query("INSERT INTO damagedItemsInvoice SET ?", [req.body.invoice], (err, result) => {
        ...
        for (let i = 0; i < req.body.invoiceContents.length; i++) {
            ...
            connection.query(`INSERT INTO damagedItemsInvoiceContents SET itemId = ?, count = ?, price = (SELECT damagedItemPrice FROM itemPrice WHERE itemPrice.itemId = ? LIMIT 1), totalPrice = (SELECT damagedItemPrice FROM itemPrice WHERE itemPrice.itemId = ? LIMIT 1) * ?, damagedItemsInvoiceId = ?`, [
