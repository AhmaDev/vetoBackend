...
router.get('/id/:id', function (req, res, next) {
    connection.query("SELECT * ,(SELECT storeName FROM customer WHERE idCustomer = damagedItemsInvoice.customerId) As customerName,DATE_FORMAT(createdAt, '%Y-%m-%d') As creationFixedDate, DATE_FORMAT(createdAt, '%T') As creationFixedTime, DATE_FORMAT(createdAt, '%W') As creationDayName ,(SELECT GROUP_CONCAT(json_object('itemId',itemId,'count',count,'idDamagedItemsInvoiceContents',idDamagedItemsInvoiceContents,'itemName',(SELECT itemName FROM item WHERE idItem = damagedItemsInvoiceContents.itemId LIMIT 1))) FROM damagedItemsInvoiceContents WHERE damagedItemsInvoiceContents.damagedItemsInvoiceId = damagedItemsInvoice.idDamagedItemsInvoice) As items FROM damagedItemsInvoice", (err, result) => {
        console.log(err);
        if (result.length > 0) {
...
router.get('/contents', function (req, res, next) {
    ...
    connection.query(`SELECT *, (SELECT @customerId := customerId FROM damagedItemsInvoice WHERE idDamagedItemsInvoice = damagedItemsInvoiceContents.damagedItemsInvoiceId) As customerId,(SELECT storeName FROM customer WHERE idCustomer = @customerId) As customerName, (SELECT itemName FROM item WHERE idItem = damagedItemsInvoiceContents.itemId) As itemName ,(SELECT imagePath FROM item WHERE idItem = damagedItemsInvoiceContents.itemId) As imagePath , DATE_FORMAT(createdAt, '%Y-%m-%d') As creationFixedDate, DATE_FORMAT(createdAt, '%T') As creationFixedTime, DATE_FORMAT(createdAt, '%W') As creationDayName , (SELECT @createdBy := createdBy FROM damagedItemsInvoice WHERE idDamagedItemsInvoice = damagedItemsInvoiceContents.damagedItemsInvoiceId) As createdBy, (SELECT username FROM user WHERE idUser = @createdBy) As createdByName FROM damagedItemsInvoiceContents WHERE 1=1 ${query} ${order} ${limit}`, (err, result) => {
        res.send(result);
        if (err) {
            console.log(err);
        }
